...
func (v *VolatilityRisk) GetRiskMetrics(ctx context.Context) (*VolatilityRiskMetrics, error) {
	...
	// Convert to []trading.Position
	posList := make([]trading.Position, len(positions))
	for i, ps := range positions {
		posList[i] = trading.Position{Symbol: ps.Symbol, Amount: ps.Amount, CostPrice: ps.CostPrice, MarketValue: ps.MarketValue}
	}
	metrics.HighVolPositions = len(v.filterHighVolPositions(ctx, posList))
