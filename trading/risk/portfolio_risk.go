...
func (p *PortfolioRisk) CheckExposure(ctx context.Context) (*ExposureReport, error) {
	...
	// 获取当前持仓
	positionsRaw := p.positionManager.GetAllPositions()
	// Convert []*PositionState to []Position
	positions := make([]trading.Position, len(positionsRaw))
	for i, ps := range positionsRaw {
		positions[i] = trading.Position{
			Symbol:      ps.Symbol,
			Amount:      ps.Amount,
