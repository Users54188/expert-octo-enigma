...
func (m *StrategyManager) ProcessSignals(ctx context.Context, signals []*Signal) error {
	...
	for _, signal := range signals {
		...
		case "buy":
			// 处理买入信号
			buySignal := &trading.TradingSignal{Action: "buy", Symbol: signal.Symbol, Confidence: signal.Strength}
	_, _ = m.signalHandler.ExecuteSignal(ctxWithTimeout, buySignal, 0.0, float64(100))
		case "sell":
			// 处理卖出信号
			sellSignal := &trading.TradingSignal{Action: "sell", Symbol: signal.Symbol, Confidence: signal.Strength}
	_, _ = m.signalHandler.ExecuteSignal(ctxWithTimeout, sellSignal, 0.0, float64(0))
		case "hold":
			// 持仓信号，不需要特殊处理
