...
func (m *MLStrategy) performMLPrediction(ctx context.Context, features map[string]float64) (*MLPrediction, error) {
	...
	// Extract feature values into slice

	featureVals := make([]float64, 0, len(features))

	for _, v := range features {

		featureVals = append(featureVals, v)

	}

	_, confidence, err := m.modelProvider.Predict(featureVals)
...
func (m *MLStrategy) calculateVolumeMA(period int) float64 {
	...
	return float64(period) * 1000000 // 模拟成交�?
