...
func (oe *OrderExecutor) recordOrder(order Order) {
	if oe.tradeHistory != nil {
		if err := oe.tradeHistory.SaveOrder(order); err != nil {
			log.Printf("保存订单失败 %s: %v", order.OrderID, err)
			return
		}
	}
	log.Printf("订单记录: %s %s %s %.2f x %d", order.OrderID, order.Symbol, order.Type, order.Price, order.Amount)
}
...
func (oe *OrderExecutor) SyncTrades(ctx context.Context) error {
	...
	for _, trade := range trades {
		// 更新持仓
		if err := oe.positionMgr.UpdatePosition(trade); err != nil {
			log.Printf("更新持仓失败 %s: %v", trade.Symbol, err)
		}

		// 记录交易历史
		if oe.tradeHistory != nil {
			if err := oe.tradeHistory.SaveTrade(TradeRecord{
				...
				log.Printf("保存交易记录失败 %s: %v", trade.TradeID, err)
			}
		}
	}
